...
## 2.5 커밋 기록 파일 (필수 · 예외 없음)

커밋 메시지와 **별도로**, 아래 경로에 md 를 **무조건** 남긴다.

| 항목 | 내용 |
|------|------|
| **경로** | **`docs/branchs/commits/` 만** |
| **금지** | `docs/commits/` · 다른 폴더 · 기록 생략 |
| **시점** | 커밋 **전** 또는 **직후** (둘 중 하나 이상 필수) |
| **파일명** | `YYMMDD_HHMM_[커밋ID]_[커밋이름]_[커밋브랜치].md` |
| **본문** | [`docs/branchs/TEMPLATE.md`](../branchs/TEMPLATE.md) |

에이전트는 커밋·푸시 전에 기록이 있는지 확인한다.
